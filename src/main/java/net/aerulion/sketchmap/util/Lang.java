package net.aerulion.sketchmap.util;

public class Lang {

  public final static String CHAT_PREFIX = "§e[§a§lSketchMap§e]§7 ";

  public static final String CONSOLE_ENABLING = CHAT_PREFIX + "§eAktiviere Plugin...";
  public static final String CONSOLE_PLUGIN_ENABLED = CHAT_PREFIX + "§eDas Plugin wurde aktiviert.";
  public static final String CONSOLE_DISABLING = CHAT_PREFIX + "§eDeaktiviere Plugin...";
  public static final String CONSOLE_PLUGIN_DISABLED =
      CHAT_PREFIX + "§eDas Plugin wurde deaktiviert.";
  public static final String CONSOLE_LOADING_SKETCHMAPS =
      CHAT_PREFIX + "§eSketchMaps werden geladen...";
  public static final String CONSOLE_SKETCHMAPS_LOADED = "§e SketchMaps wurden geladen. Dauerte §a";
}
