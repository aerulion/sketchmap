...
public class LoadSketchMapsTask extends BukkitRunnable {
    ...
    public void run() {
        ...
        try (Connection connection = MySQLUtils.getConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM `aerulion_sketchmap`");
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet != null) {
                while (resultSet.next()) {
                    try {
                        Main.LoadedSketchMaps.put(resultSet.getString("NAMESPACE_ID"), new SketchMap(resultSet.getString("UUID"), resultSet.getString("NAMESPACE_ID"), Base64Utils.decodeBufferedImage(resultSet.getString("IMAGE")), resultSet.getInt("X_PANES"), resultSet.getInt("Y_PANES"), Utils.decodeMapping(resultSet.getString("MAPPING"))));
            ...
            ConsoleUtils.sendColoredConsoleMessage(Messages.PREFIX.getRaw() + "§a" + Main.LoadedSketchMaps.size() + Messages.CONSOLE_SKETCHMAPS_LOADED.getRaw() + (System.currentTimeMillis() - startMillis) + "ms");
