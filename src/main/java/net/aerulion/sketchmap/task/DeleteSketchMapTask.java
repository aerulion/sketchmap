...
public class DeleteSketchMapTask extends BukkitRunnable {
    ...
    public void run() {
        try (Connection connection = MySQLUtils.getConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement("DELETE FROM `aerulion_sketchmap` WHERE `UUID` = ?");
            ...
            COMMANDSENDER.sendMessage(Messages.MESSAGE_SKETCHMAP_DELETED_1.get() + SKETCHMAP.getNamespaceID() + Messages.MESSAGE_SKETCHMAP_DELETED_2.getRaw());
